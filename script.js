...
function expandHex(shortHex) {
  if (shortHex.length === 4) { // e.g. #abc
    return '#' + shortHex[1].repeat(2) + shortHex[2].repeat(2) + shortHex[3].repeat(2);
  }
  return shortHex;
}
...
document.getElementById('applyColorBtn').addEventListener('click', () => {
  ...
  if (isValidHex(rawColor)) {
    const color = expandHex(rawColor).toUpperCase();
    updateColorDisplay(color);
    colorInput.value = color;
    colorPicker.value = color;
    removeSelectedHighlight();
  } else {
    errorMsg.textContent = 'Please enter a valid hex color code (e.g., #FF5733 or #FFF)';
  }
});
...
colorPanel.addEventListener('click', function (event) {
  if (cursorMode && event.target.classList.contains('color-box')) {
    const color = event.target.dataset.color.toUpperCase();
    colorInput.value = color;
    colorPicker.value = color;
    updateColorDisplay(color);
...
document.getElementById('copyColorBtn').addEventListener('click', () => {
  ...
  if (isValidHex(color)) {
    navigator.clipboard.writeText(color).then(() => {
      ...
      errorMsg.textContent = `Copied ${color} to clipboard!`;
      setTimeout(() => {
        errorMsg.textContent = '';
    ...
    errorMsg.textContent = 'No valid color to copy.';
  }
});
...
document.getElementById('resetBtn').addEventListener('click', () => {
  ...
  removeSelectedHighlight();
  cursorMode = false;
  document.getElementById('cursorBtn').textContent = 'Cursor Mode: OFF';
  colorPanel.style.cursor = 'default';
});
